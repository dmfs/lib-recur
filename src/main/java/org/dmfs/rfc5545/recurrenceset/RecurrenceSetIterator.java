...
public class RecurrenceSetIterator
{
     ...
     * instances. <p> This method already filters all exceptions, so {@link #mInstanceCache} contains only actual instances. </p>
     */
    ...
    private boolean isException(long instance)
    {
        ...
        while (mExceptionsInCache > 0)
        {
            ...
            else if (instance <= mExceptionCache[mExceptionsInCache - 1])
            {
                ...
                int pos = StaticUtils.linearSearch(mExceptionCache, mLastExceptionIndex + 1, mExceptionsInCache,
                        instance);
