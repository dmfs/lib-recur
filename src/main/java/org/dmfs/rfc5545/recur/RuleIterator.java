...
/**
 * An abstract iterator for recurrence rules. All rule part filters inherit this class. <p> This class does not implement {@link Iterator} because that requires
 * to know in advance if there are more instances to come. While this is easy to tell for a COUNT rule it's difficult for an UNTIL and requires additional
 * buffering. Also the {@link #nextSet()} method doesn't fit nicely into the {@link Iterator} interface. </p> <p> Intermediate iterators may return invalid
 * instances (like 2013-02-29). The {@link SanityFilter} will filter them all, so the last iterator always returns valid instances. </p> <p>
...
abstract class RuleIterator
{
    ...
    /**
     * Get the next instance. The instances are guaranteed to be strictly increasing in time. <p> If the last instance has been iterated, all subsequent calls
     * to this method will return {@link Long#MIN_VALUE}. </p>
     *
     ...
     * because all instances have been filtered by some rule this method will skip this interval and return the next non-empty interval. <p> <p> If the last
     * interval has been iterated, all subsequent calls to this method will return <code>null</code>. </p>
     *
