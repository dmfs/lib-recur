...
/**
 * Fast path for birthday type rules (i.e. instances that recur once a year on the same day and month). <p> Be sure to add a {@link SanityFilter} right after
...
public final class FastBirthdayIterator extends ByExpander
{
    ...
    {
        if (rule.hasPart(Part.BYDAY) || rule.hasPart(Part.BYYEARDAY) || rule.hasPart(Part.BYWEEKNO) || rule.hasPart(
                Part.BYHOUR) || rule.hasPart(Part.BYMINUTE)
        ...
        if (months != null
                && months.size() == 1
                && (days == null && (freq == Freq.MONTHLY || freq == Freq.YEARLY) || (days != null && days.size() == 1 && days
                .get(0) > 0 && (freq == Freq.MONTHLY
    ...
    void fastForward(long untilInstance)
    {
        ...
        mNextInstance = Instance.setYear(mNextInstance,
                nextYear + (Math.max(0, untilYear - nextYear) % mInterval) * mInterval);
    }
}
