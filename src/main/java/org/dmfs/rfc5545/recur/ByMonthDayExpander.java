 ...
 * </strong><a href="http://tools.ietf.org/html/rfc5545#section-3.3.10">RFC 5545</a> doesn't allow BYMONTHDAY to be used with WEEKLY rules, but RFC 2445 does. A
...
final class ByMonthDayExpander extends ByExpander
{
    ...
    public ByMonthDayExpander(RecurrenceRule rule, RuleIterator previous, CalendarMetrics calendarTools, long start)
    {
        ...
        mScope = rule.hasPart(Part.BYWEEKNO) || rule.getFreq() == Freq.WEEKLY ? (rule.hasPart(
                Part.BYMONTH) || rule.getFreq() == Freq.MONTHLY ? Scope.WEEKLY_AND_MONTHLY
                : Scope.WEEKLY)
                : Scope.MONTHLY;
                ...
                case WEEKLY_AND_MONTHLY:
                    ...
                    else if (0 < prevMonthDay2 && prevMonthDay2 <= prevMonthDays && prevMonthDay2 > prevMonthDays - 7)
                    {
                        ...
                        else
                        {
							 ...
							 */
                            if (calendarMetrics.getWeekOfYear(year - 1, calendarMetrics.getMonthsPerYear(year - 1) - 1,
                                    prevMonthDay2) == weekOfYear
                                    && (mMonths != null && StaticUtils.linearSearch(mMonths,
                                    calendarMetrics.getMonthsPerYear(year - 1) - 1) >= 0))
                            {
                                addInstance(Instance.make(year - 1, calendarMetrics.getMonthsPerYear(year - 1) - 1,
                                        prevMonthDay2, hour, minute, second));
                            }
                        }
                    }
                    break;
