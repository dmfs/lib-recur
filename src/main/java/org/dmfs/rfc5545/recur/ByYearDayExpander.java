...
final class ByYearDayExpander extends ByExpander
{
    ...
    public ByYearDayExpander(RecurrenceRule rule, RuleIterator previous, CalendarMetrics calendarMetrics, long start)
    {
        ...
        mScope = rule.getFreq() == Freq.WEEKLY || rule.hasPart(Part.BYWEEKNO) ? rule.hasPart(
                Part.BYMONTH) ? Scope.WEEKLY_AND_MONTHLY : Scope.WEEKLY : rule
    ...
    void expand(long instance, long start)
    {
        ...
        int startDayOfYear = mCalendarMetrics.getDayOfYear(Instance.year(start), Instance.month(start),
                Instance.dayOfMonth(start));
            ...
            switch (mScope)
            {
                case WEEKLY:
                {
                    ...
                    if (0 < actualDay && actualDay <= yearDays && newWeek == oldWeek)
                    {
                        ...
                        addInstance(Instance.setMonthAndDayOfMonth(year, CalendarMetrics.packedMonth(monthAndDay),
                    ...
                    else if (0 < nextYearDay && nextYearDay <= nextYearDays && nextYearDay < 7)
                    {
                        ...
                        if (newWeek == oldWeek)
                        {
                            ...
                            addInstance(Instance.make(year + 1, CalendarMetrics.packedMonth(monthAndDay),
                                    CalendarMetrics.dayOfMonth(monthAndDay), hour,
                                    minute, second));
                        }
                    }
                ...
                case WEEKLY_AND_MONTHLY:
                {
                    ...
                    if (0 < actualDay && actualDay <= yearDays && newWeek == oldWeek)
                    {
                        ...
                        {
                            addInstance(Instance.setMonthAndDayOfMonth(year, newMonth,
                                    CalendarMetrics.dayOfMonth(monthAndDay)));
                        }
                    }
                    else if (0 < nextYearDay && nextYearDay <= nextYearDays && nextYearDay < 7)
                    {
                        ...
                        if (newWeek == oldWeek && mMonths != null && StaticUtils.linearSearch(mMonths,
                                newMonth) >= 0 || mMonths == null && newMonth == month)
                        {
                            addInstance(Instance.make(year + 1, newMonth, CalendarMetrics.dayOfMonth(monthAndDay), hour,
                                    minute, second));
                        }
                    }
                    else if (0 < prevYearDay && prevYearDay <= prevYearDays && prevYearDay > prevYearDays - 7)
                ...
                case MONTHLY:
                {
                    if (0 < actualDay && actualDay <= yearDays && !(actualDay < startDayOfYear && year == Instance.year(
                            start)))
                    {
                        int monthAndDay = mCalendarMetrics.getMonthAndDayOfYearDay(year, actualDay);
                        int newMonth = CalendarMetrics.packedMonth(monthAndDay);
