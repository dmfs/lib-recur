...
public class RecurrenceProcessor
{
  ...
  private static final int MAX_ALLOWED_ITERATIONS = 100000;
  ...
  /**
   * Returns the time (millis since epoch) of the last occurrence, or -1 if the event repeats forever. If there are no occurrences (because the exrule or
   * exdates cancel all the occurrences) and the event does not repeat forever, then 0 is returned.
   * 
   * This computes a conservative estimate of the last occurrence. That is, the time of the actual last occurrence might be earlier than the time returned by
   * this method.
   * 
   * @param dtstart
   ...
   * @param recur
   *            the recurrence
   * @return an estimate of the time (in UTC milliseconds) of the last occurrence, which may be greater than the actual last occurrence
   ...
   *            the time of the first occurrence
   * @param maxtime
   *            the max possible time of the last occurrence. null means no limit
   * @param recur
   *            the recurrence
   * @return an estimate of the time (in UTC milliseconds) of the last occurrence, which may be greater than the actual last occurrence
   * @throws DateException
  ...
  public long getLastOccurence(Time dtstart, Time maxtime, RecurrenceSet recur) throws DateException
  {
    ...
    if (hasCount || recur.rdates != null || maxtime != null)
    {
      ...
      long[] dates = expand(dtstart, recur, dtstart.toMillis(false /* use isDst */) /* range start */,
        (maxtime != null) ? maxtime.toMillis(false /* use isDst */) : -1 /* range end */);
  ...
  /**
   * a -- list of values N -- number of values to use in a v -- value to check for
   */
  ...
  /**
   * a -- list of values N -- number of values to use in a v -- value to check for max -- if a value in a is negative, add that negative value to max and
   * compare that instead; this is how we deal with negative numbers being offsets from the end value
   */
  ...
  /**
   * Filter out the ones for events whose BYxxx rule is for a period greater than or equal to the period of the FREQ.
   * 
   * Returns 0 if the event should not be filtered out Returns something else (a rule number which is useful for debugging) if the event should not be
   * returned
   */
  private static int filter(EventRecurrence r, Time iterator)
  {
    ...
    if (EventRecurrence.WEEKLY >= freq)
    {
      ...
      {
        found = listContains(r.byweekno, r.byweeknoCount, iterator.getWeekNumber(), iterator.getActualMaximum(Time.WEEK_NUM));
        if (!found)
        {
          return 2;
        }
    ...
    if (EventRecurrence.DAILY >= freq)
    {
      ...
      {
        found = listContains(r.byyearday, r.byyeardayCount, iterator.yearDay, iterator.getActualMaximum(Time.YEAR_DAY));
        if (!found)
        {
          return 3;
        }
      ...
      {
        found = listContains(r.bymonthday, r.bymonthdayCount, iterator.monthDay, iterator.getActualMaximum(Time.MONTH_DAY));
        if (!found)
        {
          return 4;
        }
    ...
    {
      // BYHOUR
      found = listContains(r.byhour, r.byhourCount, iterator.hour, iterator.getActualMaximum(Time.HOUR));
      if (!found)
      {
        return 6;
      }
    ...
    {
      // BYMINUTE
      found = listContains(r.byminute, r.byminuteCount, iterator.minute, iterator.getActualMaximum(Time.MINUTE));
      if (!found)
      {
        return 7;
      }
    ...
    {
      // BYSECOND
      found = listContains(r.bysecond, r.bysecondCount, iterator.second, iterator.getActualMaximum(Time.SECOND));
      if (!found)
      {
        return 8;
      }
  ...
  /**
   * Filters out instances that don't match the BYSETPOS clause of a monthly recurrence rule. This is an awkward and inefficient way to go about it.
   * 
  ...
  {
    /*
     * Compute the day of the week for the first day of the month. "instance" has a day number and a DotW, so we compute the DotW of the 1st from that. Note
     * DotW is 0-6, where 0=SUNDAY.
     * 
     * The basic calculation is to take the instance's "day of the week" number, subtract (day of the month - 1) mod 7, and then make sure it's positive. We
     * can simplify that with some algebra.
     */
    int dotw = (instance.weekDay - instance.monthDay + 36) % 7;

    /*
     * The byday[] values are specified as bits, so we can just OR them all together.
     */
    int bydayMask = 0;
    ...
    /*
     * Generate a set according to the BYDAY rules. For each day of the month, determine if its day of the week is included. If so, append it to the day
     * set.
     */
    ...
    /*
     * Now walk through the BYSETPOS list and see if the instance is equal to any of the specified daySet entries.
     */
  ...
  /**
   * Return whether we should make this list from the BYxxx list or from the component of the iterator.
   */
  ...
  public static class DaySet
  {
    ...
    boolean get(Time iterator, int day)
    {
      ...
      if (SPEW)
      {
        Log.i(TAG, "get called with iterator=" + iterator + " " + iterator.month + "/" + iterator.monthDay + "/" + iterator.year + " day=" + day);
      }
      if (day < 1 || day > 28)
      {
        ...
        if (SPEW)
        {
          Log.i(TAG, "normalized t=" + t + " " + t.month + "/" + t.monthDay + "/" + t.year);
        }
      }

      /*
       * if (true || SPEW) { Log.i(TAG, "set t=" + t + " " + realMonth + "/" + day + "/" + realYear); }
       */
      if (realYear != mYear || realMonth != mMonth)
      {
        if (t == null)
        {
          ...
          if (SPEW)
          {
            Log.i(TAG, "set t=" + t + " " + t.month + "/" + t.monthDay + "/" + t.year + " realMonth=" + realMonth + " mMonth=" + mMonth);
          }
        }
        mYear = realYear;
        mMonth = realMonth;
    ...
    /**
     * Fill in a bit set containing the days of the month on which this will occur.
     * 
     * Only call this if the r.freq > DAILY. Otherwise, we should be processing the BYDAY, BYMONTHDAY, etc. as filters instead.
     * 
      ...
      if (count > 0)
      {
        ...
        for (i = 0; i < count; i++)
        {
          ...
          if (v == 0)
          {
            ...
            {
              if (SPEW)
                Log.i(TAG, "setting " + j + " for rule " + v + "/" + EventRecurrence.day2TimeDay(byday[i]));
              days |= 1 << j;
            }
          }
          else if (v > 0)
  ...
  /**
   * Expands the recurrence within the given range using the given dtstart value. Returns an array of longs where each element is a date in UTC milliseconds.
   * The return value is never null. If there are no dates then an array of length zero is returned.
   * 
   * @param dtstart
   *            a Time object representing the first occurrence
   * @param recur
   *            the recurrence rules, including RRULE, RDATES, EXRULE, and EXDATES
   * @param rangeStartMillis
   *            the beginning of the range to expand, in UTC milliseconds
   * @param rangeEndMillis
   *            the non-inclusive end of the range to expand, in UTC milliseconds; use -1 for the entire range.
   * @return an array of dates, each date is in UTC milliseconds
   * @throws DateException
   * @throws android.util.TimeFormatException
   *             if recur cannot be parsed
   */
  public long[] expand(Time dtstart, RecurrenceSet recur, long rangeStartMillis, long rangeEndMillis) throws DateException
  {
    ...
    if (recur.rrules != null)
    {
      for (EventRecurrence rrule : recur.rrules)
      {
        expand(dtstart, rrule, rangeStartDateValue, rangeEndDateValue, true /* add */, dtSet);
      }
    }
    ...
    if (recur.exrules != null)
    {
      ...
      {
        expand(dtstart, exrule, rangeStartDateValue, rangeEndDateValue, false /* remove */, dtSet);
      }
    }
  ...
  /**
   * Run the recurrence algorithm. Processes events defined in the local timezone of the event. Return a list of iCalendar DATETIME strings containing the
   * start date/times of the occurrences; the output times are defined in the local timezone of the event.
   * 
   * If you want all of the events, pass Long.MAX_VALUE for rangeEndDateValue. If you pass Long.MAX_VALUE for rangeEnd, and the event doesn't have a COUNT or
   * UNTIL field, you'll get a DateException.
   * 
   * @param dtstart
   *            the dtstart date as defined in RFC2445. This {@link Time} should be in the timezone of the event.
   * @param r
   *            the parsed recurrence, as defiend in RFC2445
   * @param rangeStartDateValue
   *            the first date-time you care about, inclusive
   * @param rangeEndDateValue
   *            the last date-time you care about, not inclusive (so if you care about everything up through and including Dec 22 1995, set last to Dec 23,
   *            1995 00:00:00
   * @param add
   *            Whether or not we should add to out, or remove from out.
   * @param out
   *            the TreeSet you'd like to fill with the events
   * @throws DateException
   * @throws android.util.TimeFormatException
    ...
    try
    {
      ...
      events:
      {
        while (true)
        {
          ...
          { // Give up after about 1 second of processing
          ...
          { // month
            ...
            if (useDays)
            {
              ...
              {
                /*
                 * iterator.weekDay indicates the day of the week (0-6, SU-SA). Because dayIndex might start in the middle of a week, and we're
                 * interested in treating a week as a unit, we want to move backward to the start of the week. (This could make the dayIndex
                 * negative, which will be corrected by normalization later on.)
                 * 
                 * The day that starts the week is determined by WKST, which defaults to MO.
                 * 
                 * Example: dayIndex is Tuesday the 8th, and weeks start on Thursdays. Tuesday is day 2, Thursday is day 4, so we want to move
                 * back (2 - 4 + 7) % 7 = 5 days to the previous Thursday. If weeks started on Mondays, we would only need to move back (2 - 1 +
                 * 7) % 7 = 1 day.
                 */
  ...
  /**
   * Normalizes the date fields to give a valid date, but if the time falls in the invalid window during a transition out of Daylight Saving Time when time
   * jumps forward an hour, then the "normalized" value will be invalid.
   * <p>
   ...
   *            the given year.
   * @param month
   *            the given month in the range [0,11]
  ...
  /**
   * Computes the weekday, a number in the range [0,6] where Sunday=0, from the given year, month, and day.
   * 
   * @param year
   *            the year
   * @param month
   *            the 0-based month in the range [0,11]
   * @param day
   *            the 1-based day of the month in the range [1,31]
   ...
   *            the 0-based month in the range [0,11]
   * @param day
   *            the 1-based day in the range [1,31]
  ...
  /**
   * Converts a normalized Time value to a 64-bit long. The mapping of Time values to longs provides a total ordering on the Time values so that two Time
   * values can be compared efficiently by comparing their 64-bit long values. This is faster than converting the Time values to UTC millliseconds.
   * 
   * @param normalized
   *            a Time object whose date and time fields have been normalized
   * @return a 64-bit long value that can be used for comparing and ordering dates and times represented by Time objects
   */
  private static final long normDateTimeComparisonValue(Time normalized)
  {
    ...
    return ((long) normalized.year << 26) + (normalized.month << 22) + (normalized.monthDay << 17) + (normalized.hour << 12) + (normalized.minute << 6)
      + normalized.second;
  }
